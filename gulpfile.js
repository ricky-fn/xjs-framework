...
gulp.task('optimizeJS', ['includeTemplate'], function () {
    ...
    var pagePromise = new Promise(function (resolve) {
        fs.readdir('dev/cache/js', function (err, files) {
            files.forEach(function (val) {
                if (/^Page/.test(val))
                    include.push(__dirname + '/dev/cache/js/' + val);
            });
            resolve();
        });
    });
    ...
    pagePromise.then(function () {
        new Promise(function (resolve) {
            fs.readdir('src/js/ui', function (err, files) {
                files.forEach(function (val) {
                    include.push(__dirname + '/dev/cache/js/ui/' + val);
                });
                resolve();
            });
        }).then(function () {
...
gulp.task('default', ['clean:dev'], function () {
    return gulp.src([src + 'js/lib/require.js', src + 'js/plugin/*.js'], {base: src + 'js/'})
        .pipe(gulp.dest(dest + '/assets/js/'))
        .on('end', function () {
            sequence('optimizeJS', ['imagemin', 'sass', 'json'], 'build:index', 'server');
        });
});
